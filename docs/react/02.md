...
// sibling요소내 key가 서로 다른 경우 순서만 조정(요소 재배치)
<ul>
<li key="0">0</li>
<li key="1">1</li>
<li key="2">2</li>
...
// 3 >= lastPlaceIndex 이니 이동이 필요 없음. lastPlaceIndex = 3으로 지정
// A가 oldChildNode에서의 index가 0임  
// 0 < 3(lastPlaceIndex) 이니 맨뒤로 이동.
...
11. useLayoutEffect에 오래 실행되는 스크립트 사용하지 말것.(useEffect와 달리 sync로 실행됨.)
